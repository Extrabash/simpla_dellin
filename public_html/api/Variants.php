...
class Variants extends Simpla
{
	...
	*/
	public function get_variants($filter = array())
	{
		...
		$query = $this->db->placehold("SELECT v.id, v.product_id , v.price, NULLIF(v.compare_price, 0) as compare_price, v.sku, IFNULL(v.stock, ?) as stock, (v.stock IS NULL) as infinity, v.name, v.attachment, v.width, v.length, v.height, v.weight, v.position
	...
	public function get_variant($id)
	{
		...
		// Модульная доставка 2
		$query = $this->db->placehold("SELECT v.id, v.product_id , v.price, NULLIF(v.compare_price, 0) as compare_price, v.sku, IFNULL(v.stock, ?) as stock, (v.stock IS NULL) as infinity, v.name, v.attachment, v.width, v.length, v.height, v.weight
