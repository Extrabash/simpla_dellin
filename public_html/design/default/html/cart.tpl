...
<tr>
	...
	<td class="name">
		...
		{$purchase->variant->name|escape} {$purchase->variant->width|escape} * {$purchase->variant->height|escape} * {$purchase->variant->length|escape} * {$purchase->variant->weight|escape}
	</td>
...
<ul id="deliveries">
	...
	<li>
		<div class="checkbox">
			...
			{elseif $cart->total_price >= $delivery->free_from && !$delivery->module}
				(бесплатно)
			{/if}
			</label>
			</h3>
